import type { Content, LocalizedContent, ModelData } from '../../types/contentrain'
...
export class RelationResolver {
  ...
  async resolveRelations<T extends Content | LocalizedContent>(
    ...
    if (!relations.length) return content
    ...
    const resolvedContent = content.map((item) => {
      ...
      for (const relation of relations) {
        ...
        if (!field || field.fieldType !== 'relation') continue
        ...
        if (!resolvedRelations.length) continue
